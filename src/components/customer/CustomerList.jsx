...
const CustomerList = () => {
    ...
    const { data, mutate } = useSWR(`/api/dashboard/customers/read-list?pageNo=${pageNo}&perPage=${perPage}&searchKey=${searchKey}`, Fetcher);
    ...
    const onDelete = async (id) => {
        ...
        if (Result.isConfirmed) {
            ...
            // }
            // let res=await fetch(`/api/dashboard/customers/delete?id=${id}`,options);
            ...
            //     ErrorToast("Customer Name Already Exist")
            //   }
            //   else {
                                                    ...
                                                    <tbody>
                                                        {data && data['data'].map((item, i) => (
                                                            <tr key={i}>
                                                                ...
                                                                <td><p className="text-xs mb-0">{item.Phone}</p></td>
                                                                <td><p className="text-xs mb-0">{item.Email}</p></td>
                                                                <td><p className="text-xs mb-0">{item.Address}</p></td>
                                                                <td>
                                                                    <Link href={`/CustomerCreateUpdate?id=${item['id']}`}><AiOutlineEdit className='pen' /></Link>

...
const ExpenseList = () => {
    ...
    const { data, mutate } = useSWR(`/api/dashboard/expenses/read-list?pageNo=${pageNo}&perPage=${perPage}&searchKey=${searchKey}`, Fetcher);
    ...
    const onDelete = async (id) => {
        ...
        if (Result.isConfirmed) {
            ...
            // }
            // let res=await fetch(`/api/dashboard/expenses/delete?id=${id}`,options);
                                                ...
                                                <div className='table-responsive table-section'>
                                                    ...
                                                    <thead className="bg-white">
                                                            <tr>
                                                                ...
                                                                <td className="text-uppercase">Type Name</td>
                                                        ...
                                                        <tbody>
                                                            {data && data['data'].map((item, i) => (
                                                                <tr key={i}>
                                                                    ...
                                                                    <td><p className="text-xs mb-0">{item.expense_types ? item.expense_types['Name']:""}</p></td>
                                                                    <td>
                                                                        <p className="text-xs mb-0">
                                                                        <NumericFormat value={item.Amount} displayType={'text'} thousandSeparator={true} prefix={'$'} />
                                                                        </p>
                                                                    </td>
                                                                    <td><p className="text-xs mb-0">{item.Note}</p></td>
                                                                    <td>
                                                                        <Link href={`/ExpenseCreateUpdate?id=${item['id']}`}><AiOutlineEdit className='pen' /></Link>
